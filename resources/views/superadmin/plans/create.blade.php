...
@section('title', 'Create Plan — ParkFlow')
...
@section('content')
  ...
  <div class="pf-form-panel">
    <div class="pf-form-head">
      <div class="pf-form-title">Create New Plan</div>
      <div class="pf-form-sub">Define a subscription plan that companies can purchase</div>
    </div>
      ...
      <form action="{{ route('superadmin.plans.store') }}" method="POST">
        ...
        <div class="pf-form-row">
          <div>
            ...
            <input type="text" name="name" class="pf-input @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="e.g. Starter" required>
            @error('name') <div class="pf-error">{{ $message }}</div> @enderror
          </div>
          <div>
            <label class="pf-label">Price (RWF) *</label>
            <input type="number" name="price" class="pf-input @error('price') is-invalid @enderror" value="{{ old('price') }}" min="0" step="100" required>
        ...
        <div class="pf-form-row">
          <div>
            ...
            <input type="number" name="duration_days" class="pf-input @error('duration_days') is-invalid @enderror" value="{{ old('duration_days', 30) }}" min="1" required>
            @error('duration_days') <div class="pf-error">{{ $message }}</div> @enderror
          </div>
          <div>
            <label class="pf-label">Sort Order</label>
            <input type="number" name="sort_order" class="pf-input @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', 0) }}" min="0">
        ...
        <div class="pf-form-full">
          ...
          <textarea name="description" class="pf-textarea @error('description') is-invalid @enderror" placeholder="Brief plan description...">{{ old('description') }}</textarea>
        ...
        <div class="pf-form-row">
          <div>
            ...
            <input type="number" name="max_zones" class="pf-input" value="{{ old('max_zones') }}" min="0" placeholder="Leave empty for unlimited">
          </div>
          <div>
            <label class="pf-label">Max Slots</label>
            <input type="number" name="max_slots" class="pf-input" value="{{ old('max_slots') }}" min="0" placeholder="Leave empty for unlimited">
          </div>
        </div>
        ...
        <div class="pf-form-row">
          <div>
            ...
            <input type="number" name="max_users" class="pf-input" value="{{ old('max_users') }}" min="0" placeholder="Leave empty for unlimited">
          </div>
          <div></div>
        </div>
        ...
        <div class="pf-check-row">
          <input type="checkbox" name="momo_payments" value="1" id="momo_payments" {{ old('momo_payments') ? 'checked' : '' }}>
          <label for="momo_payments" class="pf-check-label">Enable MoMo Payments</label>
        </div>

        <div class="pf-check-row">
          <input type="checkbox" name="reports_enabled" value="1" id="reports_enabled" {{ old('reports_enabled') ? 'checked' : '' }}>
