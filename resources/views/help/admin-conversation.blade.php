...
<style>
  ...
  .hc-role-tag { font-size: 11px; color: #6B7280; }
...
<div class="hc-wrap">
  ...
  <div class="hc-header">
    ...
    <div class="hc-avatar">{{ strtoupper(substr($otherUser->name, 0, 2)) }}</div>
    <div>
      <div class="hc-name">{{ $otherUser->name }}</div>
      <div class="hc-role-tag">{{ $otherUser->role->name ?? 'User' }} · {{ $otherUser->phone }}</div>
    </div>
  </div>
  ...
  <div class="hc-input-area">
    <textarea class="hc-input" id="hc-input" placeholder="Reply to {{ $otherUser->name }}..." rows="1"></textarea>
